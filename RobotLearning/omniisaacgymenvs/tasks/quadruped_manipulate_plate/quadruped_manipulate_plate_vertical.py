...
from robot.quadruped_robot import QuadrupedRobotVerticalOVFixedOmni
from tasks.base.rl_task import RLTask
...
class QuadrupedManipulatePlateVertical(RLTask):
    '''
    ...
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self.robot_manipulation.robot_description.default_position = [0.0, 0.0, 0.0]
        ...
        self.default_obj_positions_mani = torch.tensor([0.0, 0.0, 0.35], dtype=torch.float32, device=self._device).repeat((self._num_envs, 1))
        ...
        self.default_pose_indicator_mani_positions = torch.tensor([[0.0, 0.0, 0.4]], dtype=torch.float32, device=self._device).repeat((self._num_envs, 1))
        # Metrics
    ...
    def post_reset(self):
        self.robot_manipulation.post_reset_robot(self._env_pos, self._num_envs, self._time_inv, self._device)
    ...
    def pre_physics_step(self, actions):
        ...
        self.robot_manipulation.take_action(actions)

        # joint_position_targets = self.default_joint_positions_mani[:, 0:12] + 1.5 * actions
        # self.robot_manipulation.set_joint_position_targets(joint_position_targets)
    ...
    def reset_idx(self, env_ids):
        ...
        # Reset obj poses
        # obj_quaternions = torch.tensor([0.0, 0.643, 0.766, 0.0], dtype=torch.float32, device=self._device).repeat(num_reset, 1)
    ...
    def get_observations(self):
        ...
        # current_z_vec = quat_axis(self.plate_quat_robot, 2)
        # current_x_vec = quat_axis(self.plate_quat_robot, 0)
