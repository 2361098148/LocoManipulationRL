...
from tasks.quadruped_manipulate_plate.united_quadruped_manipulate_plate_inverted import UnitedQuadrupedManipulatePlateInverted
...
class UnitedQuadrupedManipulatePlateInvertedOmni(RLTask, UnitedQuadrupedManipulatePlateInverted):
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self.robot.robot_description.default_position = [0.0, 0.0, 0.0]
        self.robot.robot_description.default_quaternion = [0.0, 1.0, 0.0, 0.0]
        ...
        UnitedQuadrupedManipulatePlateInverted.__init__(self)
    ...
    def pre_physics_step(self, actions):
        UnitedQuadrupedManipulatePlateInverted.pre_physics_step(self, actions)
    ...
    def get_observations(self):
        return UnitedQuadrupedManipulatePlateInverted.get_observations(self)

    def calculate_metrics(self) -> None:
        return UnitedQuadrupedManipulatePlateInverted.calculate_metrics(self)

    def is_done(self):
        UnitedQuadrupedManipulatePlateInverted.is_done(self)
    ...
    def reset_plate_poses(self, indices):
        num_reset = len(indices)
        default_plate_positions = torch.tensor([0.0, 0.0, 0.18], dtype=torch.float32, device=self._device).repeat((num_reset, 1))
