...
class QuadrupedManipulatePlateCustomController(RLTask):
    '''
    ...
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self.default_obj_positions_mani = torch.tensor([0.0, 0.0, 0.18], dtype=torch.float32, device=self._device).repeat((self._num_envs, 1))
    ...
    def calculate_metrics(self) -> None:
        ...
        '''Mechanical power penalty'''
        mechanical_power = torch.sum(torch.abs(self.torque * self.joint_velocities_mani), dim=-1)
        ...
        position_target_error = torch.sum(torch.abs(self.last_joint_position_targets - self.joint_positions_mani), dim=-1)
