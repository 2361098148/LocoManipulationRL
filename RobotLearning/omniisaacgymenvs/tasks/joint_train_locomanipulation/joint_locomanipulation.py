...
class JointLocomanipulation(JointRLTask):
    '''
    ...
    traj_dir = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/analysis/joint_trajectories"
    joint_loco_file = os.path.join(traj_dir, "0608-mlp_joint_loo.npy")
    joint_mani_file = os.path.join(traj_dir, "0608-mlp_joint_mani.npy")
    joint_loco = []
    joint_loco_finished = False
    joint_mani = []
    joint_mani_finished = False
    ...
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self.default_obj_positions_mani = torch.tensor([0.0, 0.0, 0.68], dtype=torch.float32, device=self._device).repeat((self._single_task_num_envs, 1))
    ...
    def get_observations(self):
        ...
        # Record joint positions
        if self.RECORD_JOINT:
            if not self.joint_loco_finished:
                joint_loco_numpy = self.robot_locomotion.joint_positions.clone()[0].cpu().numpy()
                self.joint_loco.append(joint_loco_numpy)
            if not self.joint_mani_finished:
                joint_mani_numpy = self.robot_manipulation.joint_positions.clone()[0].cpu().numpy()
                self.joint_mani.append(joint_mani_numpy)
    ...
    def is_done(self):
        ...
        if self.RECORD_JOINT:
            if self.reset_buf[0] == 1:
                # locomotion task finished
                self.joint_loco_finished = True
                # Save npy
                np.save(self.joint_loco_file, self.joint_loco)
            if self.reset_buf[1] == 1:
                # manipulation task finished
                self.joint_mani_finished = True
                # Save npy
                np.save(self.joint_mani_file, self.joint_mani)

            if self.joint_loco_finished and self.joint_mani_finished:
                exit(0)