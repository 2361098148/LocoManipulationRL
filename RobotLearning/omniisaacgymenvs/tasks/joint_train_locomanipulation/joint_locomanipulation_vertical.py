...
from robot.quadruped_robot import QuadrupedRobotVerticalOVFixedOmni, QuadrupedRobotVerticalOVOmni
...
class JointLocomanipulationVertical(JointRLTask):
    '''
    ...
    min_joint_1_pos = -2.09 # For a1, 135 degree
    max_joint_1_pos = 2.09 # For a1, 45 degree
    reset_min_joint_1_pos = -2.26 # For a1, 140 degree
    reset_max_joint_1_pos = 2.26 # For a1, 50 degree

    def __init__(self, sim_config, name, env, offset=None) -> None:
        self.robot_locomotion = QuadrupedRobotVerticalOVOmni()
        ...
        self.robot_manipulation = QuadrupedRobotVerticalOVFixedOmni()
        self.robot_manipulation.robot_description.default_quaternion = [0.0, 1.0, 0.0, 0.0]
        self.robot_manipulation.robot_description.default_position = [0.0, 0.0, 0.6]
        ...
        self.__corner_pos_robot = torch.cat((
            torch.tensor([0.0, -0.115, -0.1853]).repeat(self._single_task_num_envs, 1),
            torch.tensor([0.0, 0.115, -0.1853]).repeat(self._single_task_num_envs, 1),
            torch.tensor([-0.115, 0.0, -0.1853]).repeat(self._single_task_num_envs, 1),
            torch.tensor([0.115, 0.0, -0.1853]).repeat(self._single_task_num_envs, 1)
        ...
        self.default_obj_positions_mani = torch.tensor([0.0, 0.0, 0.95], dtype=torch.float32, device=self._device).repeat((self._single_task_num_envs, 1))
        ...
        self.default_pose_indicator_loco_positions = torch.tensor([[0.0, 0.0, 0.5]], dtype=torch.float32, device=self._device).repeat((self._single_task_num_envs, 1))
        self.default_pose_indicator_mani_positions = torch.tensor([[0.0, 0.0, 1.1]], dtype=torch.float32, device=self._device).repeat((self._single_task_num_envs, 1))
        # Metrics
    ...
    def post_reset(self):
        self.robot_locomotion.post_reset_robot(self._env_pos, self._single_task_num_envs, self._time_inv, self._device)
        self.robot_manipulation.post_reset_robot(self._env_pos, self._single_task_num_envs, self._time_inv, self._device)
    ...
    def pre_physics_step(self, actions):
        ...
        actions[0:self._single_task_num_envs, :] = 0.0
        actions[self._single_task_num_envs:, :] = 0.0

        self.robot_locomotion.take_action(actions[0:self._single_task_num_envs, :])
        self.robot_manipulation.take_action(actions[self._single_task_num_envs:, :])
    ...
    def get_observations(self):
        ...
        obs_loco = torch.cat((
            ...
            self.current_actions[0:self._single_task_num_envs, :],
            self.last_actions[0:self._single_task_num_envs, :]
        ), dim=-1)
        ...
        obs_mani = torch.cat((
            ...
            scaled_joint_positions,
            scaled_joint_velocities,
            self.current_actions[self._single_task_num_envs:, :],
            self.last_actions[self._single_task_num_envs:, :]
        ), dim=-1)
    ...
    def calculate_metrics(self) -> None:
        ...
        dof1_too_low = dof1_positions < self.min_joint_1_pos
        dof1_too_high = dof1_positions > self.max_joint_1_pos
        joint1_pos_break_limit_sumup = dof1_too_low + dof1_too_high # Shape (N, 4)
