# import sys
# real_dir = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment"
# traj_dir = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/tasks"
# sys.path.append(traj_dir)
# sys.path.append(real_dir)

# from trajectory_tracker import QuadrupedTrajectoryLoader
...
from omni.isaac.core.utils.torch.rotations import quat_rotate_inverse, quat_conjugate, quat_mul, quat_axis, quat_rotate
...
class QuadrupedPoseControlCustomControllerDR(RLTask):
    '''
    ...
    base_linear_vel_scale = 1.0
    base_angular_vel_scale = 0.1
    ...
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self._num_observations = 82 # 88
        self._num_states = 82 # 88
        ...
        self._friciton_reset_range = [1.6, 2.0] # Value from 0.6-1.0
        self._friction_reset_interval = 400

        if DEBUG:
            # traj_file = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/data/trajectory_tracking/traj_turning.txt"
            
            # self.traj = QuadrupedTrajectoryLoader(traj_file,
            #                                       repeat=True)
            
            
            if RECORD:
                output_file_path = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/data/trajectory_tracking/0609-exp2"
                self.output_file = open(output_file_path, "wb+")
                self.record_finished = False
                self.recorded_trajectory = []
            else:
                traj_file = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/data/trajectory_tracking/yaw_60_sim_traj"
                with open(traj_file, "rb") as f:
                    self.recorded_trajectory = pickle.load(f)
                self.current_traj_i = 0
                self.num_traj = len(self.recorded_trajectory)

                if COMPARE:
                    self.compare_finished = False
                    real_robot_data_file = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/data/quadruped_pose_control/0530-yaw_90-add_obs"
                    with open(real_robot_data_file, "rb") as f:
                        # load the pickle object
                        self.real_motion_data = pickle.load(file=f)
                    self.obs_diff = []

                    self.compare_output_file = "/home/bionicdl/SHR/LocomanipulationTransfer/RobotLearning/real_experiment/data/quadruped_pose_control/0530-yaw_90-obs_diff"
    ...
    def pre_physics_step(self, actions):
        ...
        if ADD_NOISE:
            # Add action noise
            action_noise = 0.05 * torch.rand_like(actions)
            self.current_actions_with_noise = action_noise + actions
        else:
            self.current_actions_with_noise = self.current_actions
        ...
        delta_joint_position_targets = self.current_actions_with_noise * self.action_scale
        ...
        if DEBUG:
            # position_targets = self.traj.get_current_traj()
            # if len(position_targets) > 0:
            #     self.current_joint_position_targets = torch.tensor(position_targets, dtype=torch.float32, device=self._device).repeat(self._num_envs, 1)
            # else:
            #     self.current_joint_position_targets = self.default_joint_positions_loco.clone()[:, 0:12]
            if RECORD:
                if not self.record_finished:
                    current_joint_position_targets = self.current_joint_position_targets[0, :].clone()
                    self.recorded_trajectory.append(current_joint_position_targets)
            if REPLAY:
                if self.current_traj_i < self.num_traj:
                    current_joint_position_targets = self.recorded_trajectory[self.current_traj_i]
                    self.current_joint_position_targets = current_joint_position_targets.repeat(self._num_envs, 1)
    ...
    def reset_idx(self, env_ids):
        ...
        if not DEBUG:
            rand_deviation = 0.2*(torch.rand(num_reset, 12, dtype=torch.float32, device=self._device)-0.5)
            deviation = torch.zeros(num_reset, 20, dtype=torch.float32, device=self._device)
            deviation[:, 0:12] = rand_deviation
        else:
            deviation = 0.0
        joint_positions = self.default_joint_positions_loco[env_ids] + deviation
        self.robot_locomotion.set_joint_positions(joint_positions, indices=env_ids, full_joint_indices=True)
        ...
        if not DEBUG:
            rand_velocities_loco = 0.2*(torch.rand(num_reset, 20, dtype=torch.float32, device=self._device)-0.5)
        else:
            rand_velocities_loco = torch.zeros(num_reset, 20, dtype=torch.float32, device=self._device)
        self.robot_locomotion.set_joint_velocities(rand_velocities_loco, indices=env_ids, full_joint_indices=True)

        # Reset robot poses
        if not DEBUG:
            position_deviation = torch.rand(num_reset, 3, dtype=torch.float32, device=self._device)
            xy_deviation = 0.1*(position_deviation[:, 0:2] - 0.5)
            z_deviation = 0.1*position_deviation[:, 2].view(num_reset, 1)
            ...
            xyz_deviation = 0.0
            rand_init_quaternion = torch.tensor([1.0, 0.0, 0.0, 0.0], dtype=torch.float32, device=self._device).repeat(num_reset, 1)

        self.robot_locomotion.set_robot_pose(positions=self.default_robot_positions_loco[env_ids]+xyz_deviation,
                                             quaternions=rand_init_quaternion,
                                             indices=env_ids)
        ...
        self.pose_indicator_loco.set_object_poses(quaternions=rand_quaternions_loco,
        ...
        ground_default_quaternion = self.default_robot_quaternions_loco[env_ids]
    ...
    def get_observations(self):
        ...
        if ADD_NOISE:
            base_pos_noise = 0.02 * torch.rand_like(self.base_positions_loco)
            self.base_positions_loco_with_noise = self.base_positions_loco + base_pos_noise

            base_quaternion_noise = rand_quaternions(num_envs=self._num_envs,
                                                     min_roll=-0.2,
                                                     max_roll=0.2,
                                                     min_pitch=-0.2,
                                                     max_pitch=0.2,
                                                    ...
                                                    #  max_yaw=0.0,
                                                     device=self._device)
            self.base_quaternions_loco_with_noise = rotate_orientations(self.base_quaternions_loco,
                                                                        base_quaternion_noise,
                                                                        self._device)

            joint_vel_noise = 0.1 * torch.rand_like(self.joint_velocities_loco)
            self.joint_velocities_loco_with_noise = self.joint_velocities_loco + joint_vel_noise

            joint_pos_noise = 0.0005 * torch.rand_like(self.joint_positions_loco)
            self.joint_positions_loco_with_noise = self.joint_positions_loco + joint_pos_noise
        else:
            self.base_positions_loco_with_noise = self.base_positions_loco

            self.base_quaternions_loco_with_noise = self.base_quaternions_loco

            self.joint_velocities_loco_with_noise = self.joint_velocities_loco

            self.joint_positions_loco_with_noise = self.joint_positions_loco
        ...
        '''Scaled observations'''
        scaled_base_positions = self.base_positions_loco_with_noise * self.base_position_scale
        scaled_base_linear_vels = self.base_linear_velocities_loco * self.base_linear_vel_scale
        scaled_base_angular_vels = self.base_angular_velocities_loco * self.base_angular_vel_scale
        ...
        scaled_joint_positions = self.joint_position_scale * self.joint_positions_loco_with_noise
        scaled_joint_velocities = self.joint_velocity_scale * self.joint_velocities_loco_with_noise

        quat_diff_loco = quat_mul(self.base_quaternions_loco_with_noise, quat_conjugate(self.goal_quaternions))
        ground_up_vec = quat_axis(self.base_quaternions_loco_with_noise, 2)
        ...
        self.obs_buf[:] = torch.cat((scaled_base_positions,
                                     ground_up_vec,
                                     # scaled_ground_quaternion,
                                     quat_diff_loco,
                                     # scaled_goal_ground_quaternion,
                                     # scaled_base_linear_vels,
                                     # scaled_base_angular_vels,
        ...
        if DEBUG:
            if COMPARE:
                if not self.compare_finished:
                    real_obs = self.real_motion_data.observations[self.current_traj_i]
                    sim_obs = self.obs_buf.clone()[0, :].cpu().numpy()
                    obs_diff = np.abs(sim_obs - real_obs)
                    self.obs_diff.append(obs_diff)

    def calculate_metrics(self) -> None:
        '''Pose error penalty'''
        quat_diff = quat_mul(self.base_quaternions_loco, quat_conjugate(self.goal_quaternions))
            ...
            if RECORD:
                if not self.record_finished:
                    if self.reset_buf[0] == 1:
                        # Record should be finished
                        pickle.dump(self.recorded_trajectory, self.output_file)
                        self.record_finished = True
            if COMPARE:
                if not self.compare_finished:
                    if self.reset_buf[0] == 1:
                        with open(self.compare_output_file, "wb+") as f:
                            pickle.dump(self.obs_diff, f)
