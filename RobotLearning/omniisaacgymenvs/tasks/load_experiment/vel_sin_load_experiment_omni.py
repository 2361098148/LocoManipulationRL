...
class TrajectoryRecorder(object):
    def __init__(self, trajectory_folder, trajectory_file_name) -> None:
        ...
        # Write column names
        self.csv_writer.writerow(["time", "position", "velocity", "goal_velocity"])

    def write_trajectory(self, time, position, velocity, goal_velocity):
        self.csv_writer.writerow([time, position, velocity, goal_velocity])

    def close(self):
        self.csv_file.close()

class VelSinLoadExperimentOmni(RLTask):
    def __init__(self, sim_config, name, env, offset=None) -> None:
        ...
        self.control_ki = self._task_cfg["control"]["control_ki"]
        self.control_kd = self._task_cfg["control"]["control_kd"]
        self.max_effort = self._task_cfg["control"]["max_effort"]
        self.speed_torque_k = self._task_cfg["control"]["speed_torque_k"]
        self.stall_torque = self._task_cfg["control"]["stall_torque"]

        self.trajectory_file_folder = os.path.join(env_dir, "runs", "VelSinLoad")
        ...
        self.goal_joint_velocities = torch.zeros(self._num_envs, 1, dtype=torch.float32, device=self._device)
        self.last_joint_velocities = torch.zeros(self._num_envs, 1, dtype=torch.float32, device=self._device)
    ...
    def get_current_target(self, current_time_ms):
        ...
        velocity_target = self.traj_amplitude*np.sin(k*current_time_s)

        # return velocity_target
        return self.traj_amplitude

    def post_reset(self):
        if self.custom_controller:
            self.robot_articulation.switch_control_mode("effort")
        else:
            self.robot_articulation.switch_control_mode("velocity")
        ...
        if self.custom_controller:
            ...
            joint_damping = torch.tensor([self.control_kp], dtype=torch.float32, device=self._device).repeat(self._num_envs, 1)
            self.robot_articulation.set_gains(kds=joint_damping)
    ...
    def reset_idx(self, env_ids):
        ...
        self.traj_recorder.write_trajectory(0.0, 0.0, 0.0, 0.0)
    ...
    def pre_physics_step(self, actions):
        ...
        self.goal_joint_velocities = self.get_current_target(current_time_ms)
        goal_velocity_target = torch.tensor([self.goal_joint_velocities], dtype=torch.float32, device=self._device).repeat(self._num_envs, 1)
        for i in range(self.control_decimal):
            ...
            if not self.custom_controller:
                self.robot_articulation.set_joint_velocity_targets(goal_velocity_target)
            else:
                velocity_error = goal_velocity_target - joint_velocities
                self.error_integral += velocity_error
                joint_acc = (joint_velocities - self.last_joint_velocities)/self._dt

                torque = self.control_kp * velocity_error + self.control_ki * self.error_integral + self.control_kd * (-joint_acc)
                max_effort = torch.abs(joint_velocities) * self.speed_torque_k + self.stall_torque
                torque = torch.clamp(torque, min=-max_effort, max=max_effort)
                self.robot_articulation.set_joint_efforts(torque)

                self.last_joint_velocities = joint_velocities
    ...
    def get_observations(self):
        ...
        self.traj_recorder.write_trajectory(current_time, current_joint_position, current_joint_velocity, self.goal_joint_velocities)
